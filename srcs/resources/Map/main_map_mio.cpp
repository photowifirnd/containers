...
//checking Key compare //struct less : binary_function<T, T, bool> in my Algorithm.hpp
...
//Checking erase single element from de map
  it = mymap.begin();
  while (it != mymap.end())
  {
    std::cout << "key => " << it->first << "\tvalue => " << it->second << std::endl;
  ...
  ft::map<char, int>::iterator first = mymap.begin();
  ft::map<char, int>::iterator last = mymap.end();
