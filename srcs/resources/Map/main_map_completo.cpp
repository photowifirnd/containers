...
int main()
{
	...
	std::cout << std::endl;
	{
		...
		MyMap.insert(std::make_pair(3, 43));
	...
	std::cout << std::endl;
	{
		...
		MyMap.insert(std::make_pair(10, 57758));
		MyMap.insert(std::make_pair(100, 34));
		std::cout << "Quiero eliminar el begin() -> " << MyMap.begin()->first << std::endl;
		MyMap.erase(MyMap.begin());// Eliminando el comienzo y cambiando begin()
		for (MODE::map<int, int>::iterator it = MyMap.begin(); it != MyMap.end(); it++)
			std::cout << it->first << " => " << it->second << std::endl;
	}
	std::cout << std::endl;
