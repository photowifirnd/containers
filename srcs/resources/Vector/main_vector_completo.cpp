 ...
 * 	std::cout << std::endl;
	{
		std::vector<int> tuRange(10, 10);
		std::vector<int> tuVector(tuRange.begin(), tuRange.end());
		std::cout << tuVector.capacity() << std::endl;
		std::cout << tuVector.size() << std::endl;
	}
	*/
	...
	std::cout << std::endl;
	{
		...
		for (MODE::vector<int>::const_iterator it = --(MyVector1.end()); it != --(MyVector1.begin()); it--)
			std::cout << *it << std::endl;
	}
