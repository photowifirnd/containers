...
std::cout << "\033[33m ################## Segmentation WTF_01?? #################\033[m" << std::endl;  
	std::cout << std::endl;
	{
		MODE::list<int> MyList;
		MODE::list<int> Filler(5, 66);
		...
		std::cout << "\033[34m Printing for loop \033[0m" << std::endl;
		for (MODE::list<int>::iterator it = MyList.begin(); it != MyList.end(); ++it)
			std::cout << *it << std::endl;
	}
std::cout << "\033[33m ################## Segmentation WT F_02?? #################\033[m" << std::endl;    
	std::cout << std::endl;
	{
		MODE::list<int> MyList;
	...
	std::system("leaks container_orig");
}
